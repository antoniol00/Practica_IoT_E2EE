...
class Platform:
    ...
    def decrypt_aead(self, message, key):
    ...
    def decrypt_ae(self, message, key):
            ...
            if received_message[:-32] == b'I am device ' + msg_dec['device_id'].encode('utf-8') + b' I am going to start our communication now!':
                ...
                with app.app_context():
                    if Device.query.filter_by(id=msg_dec['device_id']).first() is not None:
                        device = Device.query.filter_by(id=msg_dec['device_id']).first()
                    ...
                    device = Device(msg_dec['device_id'], self.devices[msg_dec['device_id']]['name'],
                                    self.devices[msg_dec['device_id']]['location'], self.devices[msg_dec['device_id']]['type'],
                                    self.devices[msg_dec['device_id']]['encryption_mode'],
                                    self.devices[msg_dec['device_id']]['encryption_algorithm'],
                                    self.devices[msg_dec['device_id']]['hash_algorithm'],
        ...
        while True:
            ...
            for dev in self.devices:
                ...
                if self.devices[dev]['is_registered'] and (current_time - self.devices[dev]['last_update']).total_seconds() > 300:
                    ...
                    self.generate_new_dh_parameters(self.devices[dev]['dh_algorithm'])
                    # Send the platform public key to the device
                    platform_pk = self.platform_public_key.public_bytes(
                        encoding=Encoding.PEM, format=PublicFormat.SubjectPublicKeyInfo)
                    data = {
